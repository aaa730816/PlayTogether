...
export default class UserActivity extends Component {
    ...
    _doLoadDatas = () => {
        let {params} = this.props.navigation.state;
        fetch(global.userModuleUrl + 'myActivity' + params.type + '?userId=' + global.userId, {
            method: 'GET'
            ...
            .then(responseJson => {
                if (responseJson.success) {
                    this.setState(previous => {
                        ...
                        ConversitionUtil.quitConversition(item.id, 'activity');
                        Alert.alert(
                            '',
    ...
    onRenderItem = ({item}) => {
        return (
            <TouchableOpacity onPress={() => this._goToDate(item)} onLongPress={() => this._longPressDate(item)}>
                <View style={[MCV.dateContainer, {height: 170}]}>
                    <View style={MCV.dateFirstRow}><Text
                        style={MCV.dateTitle}>{item.title + (item.numOfPeople == 0 ? '(已满)' : '')}</Text></View>
                    ...
                    {item.location.address == '' ? <Text style={MCV.dateSite}>游戏:{item.game}</Text> :
                        <Text style={MCV.dateSite}>地点:{item.location.address}</Text>}
                </View>
            </TouchableOpacity>
        )
    }
